...
- (NSDictionary *)bitmapDescriptorWithID:(uint16_t)bitmapID error:(NSError **)errorPtr {
	...
	ReturnValueWithNoError(bitmapDescriptor, errorPtr)
}

- (void)loadBitmapWithID:(uint16_t)bitmapID buffer:(void *)pixels format:(MHK_BITMAP_FORMAT)format error:(NSError **)errorPtr {
	// get a resource descriptor
	NSDictionary *descriptor = [self resourceDescriptorWithResourceType:@"tBMP" ID:bitmapID];
	if(!descriptor) ReturnWithError(MHKErrorDomain, errUnknownID, nil, errorPtr)
	...
	ByteCount bytes_read = 0;
	OSStatus err = FSReadFork(forkRef, fsFromStart, resource_offset, sizeof(MHK_BITMAP_header), &bitmap_header, &bytes_read);
	if(err) ReturnWithError(NSOSStatusErrorDomain, err, nil, errorPtr)
	MHK_BITMAP_header_fton(&bitmap_header);
	...
	// process the pixels
	if(bitmap_header.compression_flag == MHK_BITMAP_PLAIN) err = read_raw_indexed_pixels(forkRef, resource_offset, &bitmap_header, pixels, format);
	else if(bitmap_header.compression_flag == MHK_BITMAP_COMPRESSED) err = read_compressed_indexed_pixels(forkRef, resource_offset, &bitmap_header, pixels, format);
	else ReturnWithError(MHKErrorDomain, errInvalidBitmapCompression, nil, errorPtr)
	if(err) ReturnWithError(NSOSStatusErrorDomain, err, nil, errorPtr)
