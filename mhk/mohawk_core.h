...
MHK_INLINE void MHK_chunk_header_fton(MHK_chunk_header *s) {
    s->content_length = CFSwapInt32BigToHost(s->content_length);
}
...
MHK_INLINE void MHK_rsrc_table_header_fton(MHK_rsrc_table_header *s) {
    s->count = CFSwapInt16BigToHost(s->count);
}
...
MHK_INLINE void MHK_name_table_header_fton(MHK_name_table_header *s) {
    s->count = CFSwapInt16BigToHost(s->count);
}
...
MHK_INLINE void MHK_file_table_header_fton(MHK_file_table_header *s) {
    s->count = CFSwapInt32BigToHost(s->count);
}
