...
- (NSString*)description { return [NSString stringWithFormat:@"%@: {value=%u, count=%u}", [super description], value, count]; }

- (BOOL)isEqual:(id)anObject
{
  if (anObject == nil)
    return NO;
  if (![anObject isKindOfClass:[ValueCount class]])
    return NO;

  return value == ((ValueCount*)anObject)->value;
}

- (unsigned)hash { return value; }
