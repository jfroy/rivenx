...
static void texture_provider_data_release(void* info, const void* data, size_t size) { free((void*)data); }
...
static void dump_bitmaps(MHKArchive* archive)
{
  ...
  for (NSDictionary* resourceDescriptor in bmpResources) {
    ...
    CGImageRef textureImageRef = CGImageCreate(width, height, 8, 32, width * 4, genericRGBColorSpaceRef, (CGBitmapInfo)kCGImageAlphaFirst, dataProviderRef,
...
static void dump_sounds(MHKArchive* archive, int first_pkt_only)
{
  ...
  for (NSDictionary* resourceDescriptor in wavResources) {
    ...
    if (!soundDescriptor) {
      ...
      fprintf(stderr, "an error in the %s domain with code %d (%c%c%c%c) has occured\n", [[error domain] UTF8String], code, ((char*)&code)[0],
              ((char*)&code)[1], ((char*)&code)[2], ((char*)&code)[3]);
      continue;
    }
    ...
    cch.mChunkSize = (SInt64)CFSwapInt64HostToBig(samples_size + sizeof(UInt32));
    write(fd, &cch.mChunkType, sizeof(cch.mChunkType));
    write(fd, &cch.mChunkSize, sizeof(cch.mChunkSize));
