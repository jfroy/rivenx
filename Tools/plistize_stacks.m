...
static const NSString* k_eventSelectors[] = {@"mouseDown", @"mouseDown2", @"mouseUp",          @"mouseDownOrUpOrMoved", @"mouseTrack", @"mouseMoved",
                                             @"loading",   @"leaving",    @"UNKNOWN - TYPE 8", @"rendering",            @"priming"};
...
static NSDictionary* rx_decode_riven_script(const void* script, uint32_t* scriptLength)
{
  ...
  for (currentEventIndex = 0; currentEventIndex < eventCount; currentEventIndex++) {
    ...
    // program descriptor
    NSDictionary* programDescriptor =
        [[NSDictionary alloc] initWithObjectsAndKeys:program, @"program", [NSNumber numberWithUnsignedShort:commandCount], @"opcodeCount", nil];
...
int main(int argc, const char* argv[])
{
  ...
  printf("%u CARD names, %u HSPT names, %u external names, %u variable names, %u stack names\n", [cardNames count], [hotspotNames count], [externalNames count],
         [varNames count], [stackNames count]);
