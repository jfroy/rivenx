...
- (struct rx_transition_program)_loadTransitionShaderWithName:(NSString*)name direction:(RXTransitionDirection)direction context:(CGLContextObj)cgl_ctx
{
    ...
    program.program = [[GLShaderProgramManager sharedManager] standardProgramWithFragmentShaderName:name extraSources:extraSource
        epilogueIndex:[extraSource count] context:cgl_ctx error:&error];
...
// inventory textures and interpolators
    ...
    for (GLuint inventory_i = 0; inventory_i < RX_MAX_INVENTORY_ITEMS; inventory_i++)
    {
        ...
        {
            RXOLog2(kRXLoggingGraphics, kRXLoggingLevelError, @"failed to get inventory texture descriptor for item \"%@\": %@",
                RX_INVENTORY_KEYS[inventory_i], error);
            continue;
        }
...
// card compositing
    ...
    for (GLuint i = 0; i < 1; i++)
    {
        ...
        glTexImage2D(GL_TEXTURE_RECTANGLE_ARB, 0, GL_RGBA8, kRXCardViewportSize.width, kRXCardViewportSize.height, 0,
            GL_BGRA, GL_UNSIGNED_INT_8_8_8_8_REV, NULL); glReportError();
...
// shaders
    ...
    // card shader
    _card_program = [[GLShaderProgramManager sharedManager] standardProgramWithFragmentShaderName:@"card" extraSources:nil epilogueIndex:0
        context:cgl_ctx error:&error];
    if (!_card_program)
        [self _reportShaderProgramError:error];
...
- (void)_renderCardWithTimestamp:(const CVTimeStamp*)outputTime inContext:(CGLContextObj)cgl_ctx
{
    ...
    if (r->water_fx.sfxe && !_water_sfx_disabled)
    {
        ...
        if (r->water_fx.frame_timestamp == 0 || RXTimingTimestampDelta(outputTime->hostTime, r->water_fx.frame_timestamp) >= fps_inverse)
        {
            ...
            glTexSubImage2D(GL_TEXTURE_RECTANGLE_ARB, 0, 0, 0, kRXCardViewportSize.width, kRXCardViewportSize.height,
                GL_BGRA, GL_UNSIGNED_INT_8_8_8_8_REV, _water_draw_buffer); glReportError();
...
- (void)_renderCredits:(CGLContextObj)cgl_ctx
{
    ...
    if (_credits_state == 0)
    {
        ...
        glTexImage2D(GL_TEXTURE_RECTANGLE_ARB, 0, GL_RGBA8, 360, 784, 0, GL_BGRA, GL_UNSIGNED_INT_8_8_8_8_REV, _credits_texture_buffer); glReportError();
    ...
    else if (t > 1.0f)
    {
        ...
        else if (_credits_state == 3)
        {
            ...
            glTexSubImage2D(GL_TEXTURE_RECTANGLE_ARB, 0, 0, 0, 360, 392, GL_BGRA, GL_UNSIGNED_INT_8_8_8_8_REV, _credits_texture_buffer); glReportError();
        }
        ...
        else if (_credits_state == 6)
        {
            ...
            [archive loadBitmapWithID:305 buffer:BUFFER_OFFSET(_credits_texture_buffer, 360 * 392 * 4)
                format:MHK_BGRA_UNSIGNED_INT_8_8_8_8_REV_PACKED error:NULL];
            
            glTexSubImage2D(GL_TEXTURE_RECTANGLE_ARB, 0, 0, 0, 360, 784, GL_BGRA, GL_UNSIGNED_INT_8_8_8_8_REV, _credits_texture_buffer); glReportError();
        ...
        else if (_credits_state >= 7)
        {
            ...
            {
                MHKArchive* archive = [[RXArchiveManager sharedArchiveManager] extrasArchive:NULL];
                [archive loadBitmapWithID:299 + _credits_state buffer:BUFFER_OFFSET(_credits_texture_buffer, 360 * 392 * 4)
                    format:MHK_BGRA_UNSIGNED_INT_8_8_8_8_REV_PACKED error:NULL];
            }
            else
            {
...
- (void)exportCompositeFramebuffer
{
    ...
    NSBitmapImageRep* image_rep = [[NSBitmapImageRep alloc] initWithBitmapDataPlanes:NULL
        pixelsWide:kRXCardViewportSize.width pixelsHigh:kRXCardViewportSize.height
        bitsPerSample:8 samplesPerPixel:4 hasAlpha:YES isPlanar:NO colorSpaceName:NSDeviceRGBColorSpace
        bitmapFormat:NSAlphaNonpremultipliedBitmapFormat bytesPerRow:kRXCardViewportSize.width * 4 bitsPerPixel:32];
