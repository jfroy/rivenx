...
- (NSUInteger)hash { return hash_combine([stackKey hash], cardID); }

- (BOOL)isEqual:(id)object
{
  if ([self class] != [object class])
    return NO;
  return ([stackKey isEqualToString:((RXSimpleCardDescriptor*)object)->stackKey] && cardID == ((RXSimpleCardDescriptor*)object)->cardID) ? YES : NO;
}

- (NSString*)stackKey { return stackKey; }

- (uint16_t)cardID { return cardID; }

@end

@implementation RXCardDescriptor

+ (id)descriptorWithStack:(RXStack*)stack ID:(uint16_t)cardID { return [[[RXCardDescriptor alloc] initWithStack:stack ID:cardID] autorelease]; }
...
- (NSString*)description { return [[_name retain] autorelease]; }

- (RXStack*)parent { return [[_parent retain] autorelease]; }
...
- (NSData*)data { return [[_data retain] autorelease]; }
