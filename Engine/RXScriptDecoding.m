...
static NSString* const script_keys_array[11] = {@"mouse down", @"mouse still down", @"mouse up", @"unknown 3",       @"mouse inside", @"mouse exited",
                                                @"open card",  @"close card",       @"idle",     @"start rendering", @"screen update"};
...
NSDictionary* rx_decode_riven_script(const void* script, uint32_t* script_length)
{
  ...
  for (currentEventIndex = 0; currentEventIndex < eventCount; ++currentEventIndex) {
    ...
    NSDictionary* programDescriptor =
        [[NSDictionary alloc] initWithObjectsAndKeys:program, RXScriptProgramKey, [NSNumber numberWithUnsignedShort:commandCount], RXScriptOpcodeCountKey, nil];
