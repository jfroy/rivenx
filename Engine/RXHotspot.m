...
- (void)_updateWorldFrame:(NSNotification*)notification { _world_frame = RXTransformRectCoreToWorld(_rect); }
...
- (void)_updateDescription
{
  ...
  if (_name)
    _description = [[NSString alloc] initWithFormat:@"%@ {ID=%hu, core=<%hu, %hu, %hu, %hu>, world=<%d, %d, %d, %d>}", _name, _ID, _rect.left, _rect.top,
                                                    _rect.right, _rect.bottom, (int)_world_frame.origin.x, (int)_world_frame.origin.y,
                                                    (int)_world_frame.size.width, (int)_world_frame.size.height];
  else
    _description =
        [[NSString alloc] initWithFormat:@"{ID=%hu, core=<%hu, %hu, %hu, %hu>, world=<%d, %d, %d, %d>}", _ID, _rect.left, _rect.top, _rect.right, _rect.bottom,
...
- (rx_core_rect_t)coreFrame { return _rect; }
...
- (uint16_t)cursorID { return _cursor_id; }

- (NSDictionary*)scripts { return _script; }

- (NSRect)worldFrame { return _world_frame; }

- (rx_event_t)event { return _event; }

- (void)setEvent:(rx_event_t)event { _event = event; }

- (void)enable { enabled = YES; }
