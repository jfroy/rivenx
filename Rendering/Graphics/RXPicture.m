...
- (NSString*)description { return [NSString stringWithFormat:@"%@ {texture=%@, vao=%u, index=%d}", [super description], _texture, _vao, _index]; }
...
- (void)render:(const CVTimeStamp*)output_time inContext:(CGLContextObj)cgl_ctx framebuffer:(GLuint)fbo
{
  ...
  glDrawArrays(GL_TRIANGLE_STRIP, _index, 4);
  glReportError();
}
