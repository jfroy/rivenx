...
static NSString* _kRXTransitionDirectionNames[4] = {@"RXTransitionLeft", @"RXTransitionRight", @"RXTransitionTop", @"RXTransitionBottom", };
...
- (NSString*)description
{
  if (type == RXTransitionSlide) {
    ...
    return [NSString
        stringWithFormat:@"%@ {type = RXTransitionSlide, direction = %@, pushNew = %d, pushOld = %d}", [super description], directionString, pushNew, pushOld];
  } else {
    return [NSString stringWithFormat:@"%@ {type = RXTransitionDissolve}", [super description]];
  }
}

- (BOOL)isPrimed { return (source_texture != nil) ? YES : NO; }
