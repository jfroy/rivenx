...
- (uint64_t)startTimestamp { return _start_time; }

- (double)timeLeft { return duration * (1.0 - [self progress]); }
...
- (float)valueAt:(float)t { return t; }

@end

@implementation RXCannedAnimation

- (id)initWithDuration:(double)d { return [self initWithDuration:d curve:RXAnimationCurveLinear]; }
...
- (id)initWithDuration:(double)d { return [self initWithDuration:d frequency:1.0f]; }
...
- (float)valueAt:(float)t { return 0.5f * cosf(_omega * t) + 0.5f; }
